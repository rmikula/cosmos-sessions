...
namespace CosmosGettingStartedTutorial
{
    class Program
    {
        ...
        private async Task GenerateData()
        {
            ...
            for (var i = 1; i <= 50; i++)
            {
                var id = xx.Next(int.MaxValue).ToString();
                ...
                var ff = new ServiceBusMessage(txtMsg);

                await sender.SendMessageAsync(ff);
        ...
        private async Task AddItemsToContainerAsync()
        {
            ...
            catch (CosmosException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
            {
                ...
                var session = andersenFamilyResponse.Headers.Session;
        ...
        {
            ItemResponse<Family> wakefieldFamilyResponse =
                await this.container.ReadItemAsync<Family>("Wakefield.7", new PartitionKey("Wakefield"));
